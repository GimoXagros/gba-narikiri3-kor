...
실제 결과의 소형 출력·이름 입력·저장 소비자, 910개 이름, UI 72개와 팀 표 54개, 대사·요리·소개·설명·그래픽 등 채택한 영역별 검사를 수행한다. 검사별 JSON은 결과 ROM 해시에 연결된다. `reproduction.json`에는 도구·소스·번역·라이브러리 버전과 해시를 기록한다. 비교용 한국어 ROM도 출력 폴더 안에 생성되며 공개 산출물에는 넣지 않는다. 원래 글자 비트와 독립 비교하는 아이템 123개 설명과 대사 5208개 전체의 표시 검사도 꾸러미의 필수 조건이다. 대사는 원래 이름 처리기·포맷·스크롤·키 대기를 최대/혼합 이름까지 검사하며, 게임 장면의 정상 도달 및 번역 의미 검수와 구별한다.
